# # CannedBeer #
This is a fork from mardambey/mypipe.
Main intention of this fork is to add following features to mardambey/mypipe:

1. Custom start position for binlog file/position.
STATUS => first working version.
To achieve custom start binlog position ,we just need to make few changes in application.conf file.
...
4. Dump all data from mysql and then follow the binlogs from then on in the same kafka topics.This feature is inspired from BotttledWater(similar tool for postgresSQL.)[STATUS:not yet]

PROPER DOCUMENTATION FOR CANNED_BEER........Coming Soon..!!
