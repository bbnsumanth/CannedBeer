...
abstract class AbstractMySQLBinaryLogConsumer(
  ...
  def setBinaryLogPosition(binlogFileAndPos: BinaryLogFilePosition): Unit = {
    binlogFileAndPos == BinaryLogFilePosition.current match {
      case true ⇒ {
        //no file found in tmp/mypipe for this consumer,so let the client automatically detect it from mysql.
        log.info(s"Using current master binlog position for consuming from $hostname:$port")
      }
      case false ⇒ {
        //we find a file for binlog position in tmp/mypipe,so set the position of bin log to client.
