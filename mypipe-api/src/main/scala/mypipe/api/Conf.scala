...
object Conf {
  ...
  def binlogLoadFilePosition(consumerId: String, pipeName: String): Option[BinaryLogFilePosition] = {
    try {
      ...
      val filePos = scala.io.Source.fromFile(statusFile)
        .getLines().mkString
        .split(":")
      Some(BinaryLogFilePosition(filePos(0), filePos(1).toLong))

    } catch {
      //if it cannot find the file this is called,ideally at the first time start of mypipe
  ...
  def binlogSaveFilePosition(consumerId: String, filePos: BinaryLogFilePosition, pipe: String) {
    ...
    lastBinlogFilePos.getOrElse(fileName, "").equals(filePos) match {
      case true ⇒ {
        //do nothing as file position is already saved,because we update lastBinlogFilePos only after updating the file.
      }
      case false ⇒ {
        //update the file
        ...
        writer.close()
        //update the lastBinlogFilePos inoder to avoid re writing
