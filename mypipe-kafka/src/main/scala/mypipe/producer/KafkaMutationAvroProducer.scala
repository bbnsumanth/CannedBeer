...
import mypipe.kafka.{ KafkaProducer, PROTO_MAGIC_V0 }
...
abstract class KafkaMutationAvroProducer[SchemaId](config: Config)
  ...
  /** calls the flush method of kafkaProducer
   *  @return
   */
  ...
  override def queue(input: Mutation): Boolean = {
    try {
      val mutationType = magicByte(input)
      //get schematopic from Mutation
      val schemaTopic: String = avroSchemaSubject(input)
      //get schema from schemaTopic ,using schemoRepoClient
      val schema: Schema = schemaRepoClient.getLatestSchema(schemaTopic).get
      //get schemaId from schemaTopic and aschema ,using schemoRepoClient
      val schemaId: Option[SchemaId] = schemaRepoClient.getSchemaId(schemaTopic, schema)
      //create avro record
      val records = avroRecord(input, schema)
      //serialize:convert the record into bytes and send them to KafkaProducer Queue
      records foreach (record ⇒ {
        val bytes = serialize(record, schema, schemaId.get, mutationType)
        producer.send(getKafkaTopic(input), bytes)
      })
